...
class NotebooksListViewController: UIViewController, UITableViewDataSource {
    ...
    fileprivate func setupFetchedResultsController() {
        ...
        fetchedResultsController = NSFetchedResultsController(fetchRequest: fetchRequest, managedObjectContext: dataController.viewContext, sectionNameKeyPath: nil, cacheName: "notebooks") // by using cache to speed up especially big data
        fetchedResultsController.delegate = self
        
        do {
            try fetchedResultsController.performFetch()  //load data and start tracking
        } catch {
    ...
    func addNotebook(name: String) {
        ...
        try? dataController.viewContext.save() // save to persistant store
        
     //   reloadNotebooks() // update UI
...
//        updateEditButtonState()
   ...
   /* fileprivate func reloadNotebooks() {
    ...
    func deleteNotebook(at indexPath: IndexPath) {
        ...
        try? dataController.viewContext.save() // save this deletion into persistant store
        ...
        updateEditButtonState()*/
        //fetchedResultsController handle above already (update UI thing)
    }
    ...
    func numberOfSections(in tableView: UITableView) -> Int {
        /*if let sections = fetchedResultsController.sections {
            ...
            return 1
        }*/ //equal to below "no coalescent operator"
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        //each section has a property name numberOfObjects
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if let vc = segue.destination as? NotesListViewController {
            if let indexPath = tableView.indexPathForSelectedRow {
                ...
                vc.dataController = dataController // CoreDataStack
            }
        }
    }
}
...
extension NotebooksListViewController : NSFetchedResultsControllerDelegate {
    ...
    func controller(_ controller: NSFetchedResultsController<NSFetchRequestResult>, didChange anObject: Any, at indexPath: IndexPath?, for type: NSFetchedResultsChangeType, newIndexPath: IndexPath?) {
        switch type {
        case .insert:
            tableView.insertRows(at: [newIndexPath!], with: .fade) //.insertRows only accept array so we make newIndexPath! as a single element array
            break
        case .delete:
            tableView.deleteRows(at: [indexPath!], with: .fade)
//        default:
