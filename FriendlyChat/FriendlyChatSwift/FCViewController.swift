...
class FCViewController: UIViewController, UINavigationControllerDelegate {
    ...
    func sendMessage(data: [String:String]) {
        var mdata = data //plus the key=name & value=displayName
        mdata[Constants.MessageFields.name] = displayName
        //  create method that pushes message to the firebase database
        //childByAutoID generated a unique key by prefixed with timestamp like specifing /message/[some_autoUnique_ID]
...
extension FCViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let messageSnapshot = messages[indexPath.row] //let messageSnapshot: DataSnapshot! = messages[indexPath.row]
        let message = messageSnapshot.value as! [String: String]
        let name = message[Constants.MessageFields.name] ?? "[username]"
       
        // if image message, then grab image and display it  [update cell to display message data]
