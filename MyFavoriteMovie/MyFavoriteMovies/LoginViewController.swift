...
class LoginViewController: UIViewController {
    ...
    private func getRequestToken() {
        ...
        let task = appDelegate.sharedSession.dataTask(with: request) { (data, response, error) in
            ...
            guard (error == nil) else {
                displayError("there are an error with your request: \(String(describing: error))")
                return
            }
            
            guard let statusCode = (response as? HTTPURLResponse)?.statusCode, statusCode >= 200 && statusCode <= 299 else{
                displayError("Your request returned ad status code other than 2xx!")
                return
            }
            ...
            /*Did TMDB return an error?*/
            if let _ = parsedResult[Constants.TMDBResponseKeys.StatusCode] as? Int {
                displayError("TMDB returned an error. See the '\(Constants.TMDBResponseKeys.StatusCode)' and '\(Constants.TMDBResponseKeys.StatusMessage)' in \(parsedResult)")
                return
            }
            ...
            /* 6. Use the data! */
            //print(requestToken) // Debug
            self.appDelegate.requestToken = requestToken
            self.loginWithToken(self.appDelegate.requestToken!)
            
        }
        print("!!!!!!!1234567890__task=\(task)") // Debug
    ...
    private func loginWithToken(_ requestToken: String) {
        ...
        let task = appDelegate.sharedSession.dataTask(with: request) { (data, response, error) in
            ...
            guard (error == nil) else {
                displayError("There are an error with your request: \(error!)")
                return
            }
